/* ------------------------------------------------------------------
   ...
   Hover animations: classes kb-* / pa-* run only while the card
   (the .group link) is hovered; see globals.css.
------------------------------------------------------------------- */
...
function Commerce() {
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden>
      ...
      <rect width="400" height="300" fill={ground} />
      {/* renewal loop, slowly turning on hover */}
      ...
      {[-16, 0, 16].map((rot, i) => (
        <g key={rot} transform={`rotate(${rot} 200 190)`}>
          <g className="kb-bob" style={d(i * 0.15)}>
      ...
      {[
        ...
        <circle key={i} className="kb-bob" style={d(0.2 + i * 0.25)} cx={x} cy={y} r="7" fill="#ffd166" stroke={ink} strokeOpacity="0.35" />
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden>
      ...
      {[70, 110, 90, 150, 190].map((h, i) => (
        <rect key={i} className="kb-grow" style={d(i * 0.12)} x={250 + i * 24} y={280 - h} width="14" height={h} rx="3" fill="#e6fbf6" opacity={0.55 + i * 0.1} />
      ))}
      <path
        className="kb-trace"
        pathLength={100}
        d="M40 240 L110 205 L170 215 L230 150 L300 120 L360 60"
        fill="none"
        stroke={ink}
        strokeWidth="1.6"
        markerEnd="url(#a-bi)"
      ...
      {[
        ...
        <g key={i}>
          ...
          <circle className="kb-ping" style={d(i * 0.2)} cx={x} cy={y} r="4.5" fill="#e6fbf6" stroke={ink} strokeWidth="1.2" />
        </g>
      ))}
    </svg>
  );
...
function Marketplace() {
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden>
      ...
      <rect width="400" height="300" fill={ground} />
      {cells.map((c, i) => (
        <g key={i} className="kb-blink" style={d((i % 5) * 0.18)}>
      ...
      <g className="pa-dash" fill="none" stroke={ink} strokeWidth="1.3" strokeDasharray="4 4">
      ...
      <circle className="pa-spin-slow" cx="314" cy="150" r="38" fill="none" stroke={ground} strokeWidth="1.3" strokeDasharray="3 5" />
      <circle className="kb-ping" cx="314" cy="150" r="16" fill="#ff5c95" />
    </svg>
  );
}
...
function Fleet() {
  ...
  return (
    <svg viewBox="0 0 400 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden>
      ...
      <path className="pa-dash" d={route} fill="none" stroke={ink} strokeWidth="1.4" strokeDasharray="8 8" strokeOpacity="0.6" />
      {[
        [96, 204],
        [230, 118],
        [342, 66],
        ...
        <g key={i} transform={`translate(${x} ${y - 28})`}>
          <g className="kb-bounce" style={d(i * 0.2)}>
